...
if(isset($_REQUEST['year'])){
	$input_year=$_REQUEST['year'];
		...
		$query=$db->prepare("select * from tred_lisence_application WHERE treadlisence_year=?");
		$query->execute(array($input_year));
		$result=$query->rowCount();
		if($result==0) {
			header("location:tdl_list_by_year.php?error=1");
		}
	...
	header("location:tdl_list_by_year.php?error=0");
	}
...
<h3 class="center"><?php echo $input_year;?> সালের সকল ট্রেড লাইসেন্স  তালিকা</h3> 
...
<table border="1" cellspacing="0" cellpadding="5" id="view_form">
	<tr>
 	...
 	<?php
			...
			$statement = $db->prepare("SELECT * FROM tred_lisence_application where treadlisence_year='$input_year' ORDER BY app_id ASC");
			$statement->execute();
			...
			$statement = $db->prepare("SELECT * FROM tred_lisence_application where treadlisence_year='$input_year' ORDER BY app_id ASC LIMIT $start, $limit");
			$statement->execute();
