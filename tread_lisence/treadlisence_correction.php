...
<?php 
if(isset($_REQUEST['tdl_no'])){
	$input_app_no=$_REQUEST['tdl_no'];
		...
		if($num==0) 
		{
			...
			header("location:tdl_correction.php?error=1");
		}
	...
	header("location:tdl_correction.php?error=0");
}
...
?>
	...
	<?php
		$query=$db->prepare("select * from tred_lisence_application where app_treadlisence_no='$input_app_no'");
		$query->execute();
...
<div id="main_form">
	...
	<table width="100%" cellspacing="0" cellpadding="2" class="app_header">
		<tr>
			<td><label for="">ক্রমিক নং :  </label><strong><?php echo $row['app_serial'];?></strong>
			</td>
			<td>
	...
	<table cellspacing="0" cellpadding="2">
		...
		<h1 class="center"><a href="edit.php?id=<?php echo $row['app_serial'];?>" class="btn btn-info" >সংযোজন করুন</a>
