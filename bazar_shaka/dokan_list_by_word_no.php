...
if(isset($_REQUEST['word_no'])){
$input_market_name=$_REQUEST['word_no'];
		$query=$db->prepare("select * from market_data WHERE word_no=?");
		$query->execute(array($input_market_name));
		$result=$query->rowCount();
		if($result==0) {
			header("location:view_list_by_word_no.php?error=1");
		}
	...
	header("location:view_list_by_word_no.php?error=0");
}
...
<h3 class="center"><?php echo $input_market_name;?>  ওর্য়াড এর সকল দোকানের তালিকা</h3>
...
<table border="1" cellspacing="0" cellpadding="1" id="view_form">
	<tr>
 	...
 	<?php
			...
			$statement = $db->prepare("SELECT * FROM market_info where word_no='$input_market_name' ORDER BY m_id ASC");
			$statement->execute();
			...
			$statement = $db->prepare("SELECT * FROM market_info where word_no='$input_market_name' ORDER BY m_id ASC LIMIT $start, $limit");
			$statement->execute();
