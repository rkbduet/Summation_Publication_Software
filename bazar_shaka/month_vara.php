...
if(isset($_POST['form1'])) {
	...
	if($result==0) {
		...
		if(empty($_POST['month_name'])) {
			throw new Exception( 'অনুগ্রহ করে  মাসের নির্বাচন করুন');
		}
		if(empty($_POST['year'])) {
		...
		$query=$db->prepare("insert into month_vara(market_name,dokan_no,vara,vara_kothai,month_name,year_name,joma_date) values(?,?,?,?,?,?,?)");
		$query->execute(array($_POST['market_name'],$_POST['d_no'],$_POST['vara'],$_POST['taka_kothai'],$month_names,$_POST['year'],$date));
...
<div class="container">
<br>
<h2 class="title">Sells Management </h2>
    <?php  
