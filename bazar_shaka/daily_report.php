...
$input_date=$_REQUEST['date'];
		...
		$query=$db->prepare("select * from month_vara WHERE joma_date=?");
		$query->execute(array($input_date));
		$result=$query->rowCount();
		if($result==0) {
			header("location:day_report.php?error=1");
		}
	...
	header("location:day_report.php?=error=0");
}
...
<table border="1" cellspacing="0" cellpadding="1">
	<tr>
 	...
 	<?php
			...
			where joma_date='$input_date' ORDER BY m_id ASC");
			$statement->execute();
			...
			where joma_date='$input_date' ORDER BY m_id ASC LIMIT $start, $limit");
			$statement->execute();
 ...
 <td></td>
	<td>মোট ভাড়া জমা:</td>
		<td>
		<?php 
			$query=$db->query("select SUM(vara) as total_vara from month_vara where joma_date='$input_date'");
			$query->execute();
