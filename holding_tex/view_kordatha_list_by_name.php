...
<?php include("header.php");?>
	...
	$query=$db->prepare("select * from kordatha_info where name=?");
	$query->execute(array($name));
	$result=$query->rowCount();
	if($result==0){
		
		header("location:search_by_name.php?error=1");
		
	}
		
	}
	else{
			header("location:search_by_name.php?error=0");
...
<tr>
 	...
 	<?php
			...
			$statement = $db->prepare("SELECT * FROM kordatha_info where name='$name' ORDER BY k_id ASC");
			$statement->execute();
			...
			$statement = $db->prepare("SELECT * FROM kordatha_info where name='$name' ORDER BY k_id ASC LIMIT $start, $limit");
			$statement->execute();
