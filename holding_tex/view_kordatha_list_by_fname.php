...
<?php include("header.php");?>
	...
	$query=$db->prepare("select * from kordatha_info where f_name=?");
	$query->execute(array($f_name));
	$result=$query->rowCount();
	if($result==0){
		
		header("location:search_by_fname.php?error=1");
		
	}
		
	}
	else{header("location:search_by_fname.php?error=0");}
...
<h3 class="center">নাম অনুসারে করদাতার তালিকা </h3>
...
<tr>
 	...
 	<?php
			...
			$statement = $db->prepare("SELECT * FROM kordatha_info where f_name='$f_name' ORDER BY k_id ASC");
			$statement->execute();
			...
			$statement = $db->prepare("SELECT * FROM kordatha_info where f_name='$f_name' ORDER BY k_id ASC LIMIT $start, $limit");
			$statement->execute();
