...
<?php include("header.php");?>
	...
	$query=$db->prepare("select * from kordatha_info where vill_name=?");
	$query->execute(array($v_name));
	$result=$query->rowCount();
	if($result==0){
		
		header("location:search_by_roadno.php?error=1");
		
	}
		
	}
	else{header("location:search_by_roadno.php?error=0");}
...
<h3 class="center">এলাকা/রাস্তার নাম অনুসারে করদাতার তালিকা </h3>
...
<tr>
 	...
 	<?php
			...
			$statement = $db->prepare("SELECT * FROM kordatha_info where vill_name='$v_name' ORDER BY k_id ASC");
			$statement->execute();
			...
			$statement = $db->prepare("SELECT * FROM kordatha_info where vill_name='$v_name' ORDER BY k_id ASC LIMIT $start, $limit");
			$statement->execute();
