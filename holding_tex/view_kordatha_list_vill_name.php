...
<?php include("header.php");?>
	...
	if(isset($_REQUEST['word_no'])){
		...
		$v_name=$_REQUEST['word_no'];
		
	$query=$db->prepare("select * from kordatha_info where word_no=?");
	$query->execute(array($v_name));
	$result=$query->rowCount();
	if($result==0){
		
		header("location:holding_list_by_road_name.php?error=1");
	...
	header("location:holding_list_by_road_name.php?error=0");
...
<tr>
 	...
 	<?php
			...
			$statement = $db->prepare("SELECT * FROM kordatha_info where word_no='$v_name' ORDER BY k_id ASC");
			$statement->execute();
			...
			$statement = $db->prepare("SELECT * FROM kordatha_info where word_no='$v_name' ORDER BY k_id ASC LIMIT $start, $limit");
			$statement->execute();
